...
const TodaysSchedule: FC = () => {
    ...
    return <div className="px-4 py-2 flex flex-col flex-1" >
        ...
        <div className='flex flex-col gap-2 flex-1'>
            {schedule.filter(x => new Date(x.end_time) > new Date()).length > 0 ?
                schedule.sort((z, y) => { return (new Date(z.start_time) - new Date(y.start_time)) }).filter(x => new Date(x.end_time) > new Date()).map((item: any) => {
                        return (
