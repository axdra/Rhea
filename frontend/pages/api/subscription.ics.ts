    ...
    if (!Array.isArray(calendarCode) && calendarCode) {
        ...
        const events = await getSchema(calendarCode)
            ...
            const calEvents = events?.events.map((event:any) => {
                const start = moment(event.start_time).format('YYYY-M-D-H-m').split("-").map((item, index) => { return parseInt(item) });
                const end = moment(event.end_time).format('YYYY-M-D-H-m').split("-").map((item, index) => { return parseInt(item) });
                let url = "";
