  ...
  if (user) {
    return <div className='flex justify-center pt-24 flex-1 bg-splash dark:bg-splash-dark pb-20  '>
      <div className='max-w-[100rem] w-full flex  px-10 flex-col gap-10 ' >
        <h1 className='text-5xl font-medium break-words '>{Greet(user.email!.split('@')[0].split('.').map(x => { return x[0].toUpperCase() + x.slice(1) }).join(' '), t)}</h1>
        <div className='grid 2xl:grid-cols-5 sm:grid-cols-2 grid-cols-1  w-full flex-1 gap-5'>
          ...
          <div className='2xl:col-span-3 sm:col-span-2 col-span-1 border-black border-2 bg-white h-96 rounded-xl'>
