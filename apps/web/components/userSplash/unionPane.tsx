            ...
            props.events.map((event) => {
                return <div className="w-1/4 rounded-lg dark:bg-neutral-900/20  h-full p-4 border-2 dark:border-white/20 justify-between flex flex-col  relative group cursor-pointer">
                    ...
                    <h1 className="text-lg font-bold">{event.title}</h1>
                            ...
                            >{event.union.name}</h2>
                    {
                        event.cover_image ? <img className="w-full object-cover h-24 rounded-lg border-2 dark:border-white" src={event.cover_image} /> : <div className="w-full h-24 rounded-lg border-2 dark:border-white" style={{
